...
import Config from 'deploy/storage/Config'
...
describe('Borrowing And Funding Fee Process', function () {
  ...
  it('Case1: ethUsd borrowing fee test', async function () {
    ...
    expect(initialMargin1 * BigInt(10 - 1)).to.equals(poolInfo1.baseTokenBalance.holdAmount)
    ...
    expect(initialMargin1 * BigInt(1800)).to.equals(positionInfo1.initialMarginInUsd)
    expect(initialMargin1 * BigInt(1800)).to.equals(positionInfo1.initialMarginInUsdFromBalance)
    ...
    expect(initialMargin1 * BigInt(10) * BigInt(1800)).to.equals(positionInfo1.qty)
    expect(initialMargin1 * BigInt(10 - 1)).to.equals(positionInfo1.holdPoolAmount)
    expect(-tradeFee1 * BigInt(1800)).to.equals(positionInfo1.realizedPnl)

    // Market
    const marketInfo1 = await marketFacet.getMarketInfo(ethUsd, oracles.format(ethOracle1))
    expect(initialMargin1 * BigInt(10) * BigInt(1800)).to.equals(marketInfo1.longPositionInterest)
    ...
    const borrowingPerTokenDelta =
      BigInt(time2 - time1) *
      ((poolConfig.baseInterestRate * poolInfo1.baseTokenBalance.holdAmount) / poolInfo1.baseTokenBalance.amount)

    const realizedBorrowingFee =
      (borrowingPerTokenDelta * precision.mulRate(initialMargin1, precision.rate(10 - 1))) / BigInt(10 ** 18)
    const realizedBorrowingFeeInUsd = precision.mulPrice(realizedBorrowingFee, ethPrice1)
    ...
    expect((initialMargin1 + initialMargin2) * BigInt(10 - 1)).to.equals(poolInfo2.baseTokenBalance.holdAmount)
    expect(time2).to.equals(poolInfo2.borrowingFee.lastUpdateTime)
    expect(borrowingPerTokenDelta).to.equals(poolInfo2.borrowingFee.cumulativeBorrowingFeePerToken)
    expect(0).to.equals(poolInfo2.borrowingFee.totalBorrowingFee)
    expect(realizedBorrowingFee).to.equals(poolInfo2.borrowingFee.totalRealizedBorrowingFee)

    // Position
    const positionInfo2 = await positionFacet.getSinglePosition(user0.address, ethUsd, wethAddr, defaultMarginMode)
    expect(initialMargin1 + initialMargin2).to.equals(positionInfo2.initialMargin)
    expect((initialMargin1 + initialMargin2) * BigInt(1800)).to.equals(positionInfo2.initialMarginInUsd)
    expect((initialMargin1 + initialMargin2) * BigInt(1800)).to.equals(positionInfo2.initialMarginInUsdFromBalance)
    expect((initialMargin1 + initialMargin2) * BigInt(10) * BigInt(1800)).to.equals(positionInfo2.qty)
    expect((initialMargin1 + initialMargin2) * BigInt(10 - 1)).to.equals(positionInfo2.holdPoolAmount)
    expect((-tradeFee1 - tradeFee2) * BigInt(1800)).to.equals(positionInfo2.realizedPnl)
    expect(borrowingPerTokenDelta).to.equals(positionInfo2.positionFee.openBorrowingFeePerToken)
    expect(realizedBorrowingFee).to.equals(positionInfo2.positionFee.realizedBorrowingFee)
    expect(realizedBorrowingFeeInUsd).to.equals(positionInfo2.positionFee.realizedBorrowingFeeInUsd)

    // Market
    const marketInfo = await marketFacet.getMarketInfo(ethUsd, oracles.format(ethOracle1))
    expect((initialMargin1 + initialMargin2) * BigInt(10) * BigInt(1800)).to.equals(marketInfo.longPositionInterest)
  ...
  it('Case2: ethUsd borrowing & funding fee test', async function () {
    ...
    const orderMargin1 = precision.token(1, 17) // 0.1ETH
    const ethPrice1 = precision.price(1800)
    const oracle1 = [{ token: wethAddr, minPrice: ethPrice1, maxPrice: ethPrice1 }]
    const executionFee = precision.token(2, 15)
    ...
    // Position
    const defaultMarginMode = false
    const user0PositionInfo1 = await positionFacet.getSinglePosition(user0.address, ethUsd, wethAddr, defaultMarginMode)
    expect(0).to.equals(user0PositionInfo1.positionFee.openBorrowingFeePerToken)
    expect(0).to.equals(user0PositionInfo1.positionFee.openFundingFeePerQty)

    // Market
    const marketInfo1 = await marketFacet.getMarketInfo(ethUsd, oracles.format(oracle1))
    ...
    expect(time2).to.equals(pool.getUsdPoolBorrowingFee(usdPoolInfo2, usdcAddr)?.lastUpdateTime)

    // Position
    const user1PositionInfo2 = await positionFacet.getSinglePosition(user0.address, ethUsd, usdcAddr, defaultMarginMode)
    expect(0).to.equals(user1PositionInfo2.positionFee.openBorrowingFeePerToken)
    expect(0).to.equals(user1PositionInfo2.positionFee.openFundingFeePerQty)
    ...
    const ethPrice3 = precision.price(1830)
    ...
    const realizedEthBorrowingFee =
      (ethBorrowingPerTokenDelta * precision.mulRate(initialMargin1, precision.rate(10 - 1))) / BigInt(10 ** 18)
    const realizedEthBorrowingFeeInUsd = precision.mulPrice(realizedEthBorrowingFee, ethPrice3)

    // ethFundingRatePerSecond < 0
    const ethFundingRatePerSecond =
      (config.tradeConfig.fundingFeeBaseRate * (user0PositionInfo1.qty - user1PositionInfo2.qty)) /
      (user1PositionInfo2.qty + user0PositionInfo1.qty)
    const fundingFeeInUsd = user1PositionInfo2.qty * ethFundingRatePerSecond * BigInt(time3 - time2)
    const longFundingFeePerQtyDelta = precision.divPrice(-fundingFeeInUsd / user0PositionInfo1.qty, ethPrice3)
    const shortFundingFeePerQtyDelta = fundingFeeInUsd / user1PositionInfo2.qty
    const realizedEthFundingFeeDelta = (user0PositionInfo1.qty * longFundingFeePerQtyDelta) / BigInt(10 ** 18)
    // pool
    const poolInfo3 = await poolFacet.getPool(xEth)
    expect(time3).to.equals(poolInfo3.borrowingFee.lastUpdateTime)
    expect(ethBorrowingPerTokenDelta).to.equals(poolInfo3.borrowingFee.cumulativeBorrowingFeePerToken)
    expect(realizedEthBorrowingFee).to.equals(poolInfo3.borrowingFee.totalRealizedBorrowingFee)
    expect(0).to.equals(poolInfo3.borrowingFee.totalBorrowingFee)
    expect(realizedEthFundingFeeDelta).to.equals(poolInfo3.baseTokenBalance.unsettledAmount)

    const usdPoolInfo3 = await poolFacet.getUsdPool()
    expect(time2).to.equals(pool.getUsdPoolBorrowingFee(usdPoolInfo3, usdcAddr)?.lastUpdateTime)

    // Position
    const user0PositionInfo3 = await positionFacet.getSinglePosition(user0.address, ethUsd, wethAddr, defaultMarginMode)
    expect(ethBorrowingPerTokenDelta).to.equals(user0PositionInfo3.positionFee.openBorrowingFeePerToken)
    expect(realizedEthBorrowingFee).to.equals(user0PositionInfo3.positionFee.realizedBorrowingFee)
    expect(realizedEthBorrowingFeeInUsd).to.equals(user0PositionInfo3.positionFee.realizedBorrowingFeeInUsd)
    expect(longFundingFeePerQtyDelta).to.equals(user0PositionInfo3.positionFee.openFundingFeePerQty)

    const user1PositionInfo3 = await positionFacet.getSinglePosition(user0.address, ethUsd, usdcAddr, defaultMarginMode)
    expect(0).to.equals(user1PositionInfo3.positionFee.openBorrowingFeePerToken)
    ...
    expect(longFundingFeePerQtyDelta).to.equals(marketInfo3.fundingFee.longFundingFeePerQty)
    expect(shortFundingFeePerQtyDelta).to.equals(marketInfo3.fundingFee.shortFundingFeePerQty)
    expect(realizedEthFundingFeeDelta).to.equals(marketInfo3.fundingFee.totalLongFundingFee)
    ...
    const realizedUsdcBorrowingFee =
      (usdcBorrowingPerTokenDelta * precision.mulRate(initialMargin2, precision.rate(10 - 1))) / BigInt(10 ** 18)
    const realizedUsdcBorrowingFeeInUsd = realizedUsdcBorrowingFee

    // usdcFundingRatePerSecond < 0
    const usdcFundingRatePerSecond =
      (config.tradeConfig.fundingFeeBaseRate * (user0PositionInfo3.qty - user1PositionInfo2.qty)) /
      (user1PositionInfo2.qty + user0PositionInfo3.qty)
    const fundingFeeInUsd4 = user1PositionInfo2.qty * usdcFundingRatePerSecond * BigInt(time4 - time3)
    const longFundingFeePerQtyDelta4 =
      longFundingFeePerQtyDelta + precision.divPrice(-fundingFeeInUsd4 / user0PositionInfo3.qty, ethPrice4)
    const shortFundingFeePerQtyDelta4 = shortFundingFeePerQtyDelta + fundingFeeInUsd4 / user1PositionInfo2.qty
    const realizedUsdcFundingFeeDelta = (user1PositionInfo2.qty * shortFundingFeePerQtyDelta4) / BigInt(10 ** 18)

    // pool
    const poolInfo4 = await poolFacet.getPool(xEth)
    expect(time3).to.equals(poolInfo4.borrowingFee.lastUpdateTime)
    expect(ethBorrowingPerTokenDelta).to.equals(poolInfo4.borrowingFee.cumulativeBorrowingFeePerToken)
    expect(realizedEthBorrowingFee).to.equals(poolInfo4.borrowingFee.totalRealizedBorrowingFee)
    expect(0).to.equals(poolInfo4.borrowingFee.totalBorrowingFee)
    expect(realizedEthFundingFeeDelta).to.equals(poolInfo4.baseTokenBalance.unsettledAmount)

    expect(realizedUsdcFundingFeeDelta).to.equals(pool.getPoolStableTokenUnsettledAmount(poolInfo4, usdcAddr))

    const usdPoolInfo4 = await poolFacet.getUsdPool()
    expect(time4).to.equals(pool.getUsdPoolBorrowingFee(usdPoolInfo4, usdcAddr)?.lastUpdateTime)

    // Position
    const user0PositionInfo4 = await positionFacet.getSinglePosition(user0.address, ethUsd, wethAddr, defaultMarginMode)
    expect(ethBorrowingPerTokenDelta).to.equals(user0PositionInfo4.positionFee.openBorrowingFeePerToken)
    expect(realizedEthBorrowingFee).to.equals(user0PositionInfo4.positionFee.realizedBorrowingFee)
    expect(realizedEthBorrowingFeeInUsd).to.equals(user0PositionInfo4.positionFee.realizedBorrowingFeeInUsd)
    expect(longFundingFeePerQtyDelta).to.equals(user0PositionInfo4.positionFee.openFundingFeePerQty)

    const user1PositionInfo4 = await positionFacet.getSinglePosition(user0.address, ethUsd, usdcAddr, defaultMarginMode)
    expect(usdcBorrowingPerTokenDelta).to.equals(user1PositionInfo4.positionFee.openBorrowingFeePerToken)
    expect(shortFundingFeePerQtyDelta4).to.equals(user1PositionInfo4.positionFee.openFundingFeePerQty)

    // Market
    const marketInfo4 = await marketFacet.getMarketInfo(ethUsd, oracles.format(oracle4))
    expect(time4).to.equals(marketInfo4.fundingFee.lastUpdateTime)
    expect(longFundingFeePerQtyDelta4).to.equals(marketInfo4.fundingFee.longFundingFeePerQty)
    expect(shortFundingFeePerQtyDelta4).to.equals(marketInfo4.fundingFee.shortFundingFeePerQty)
    expect(realizedEthFundingFeeDelta).to.equals(marketInfo4.fundingFee.totalLongFundingFee)
    expect(realizedUsdcFundingFeeDelta).to.equals(marketInfo4.fundingFee.totalShortFundingFee)
  })
})
