...
describe("Decrease Market Order Process", function () {
  ...
  it("Case1: Place Single Decrease Market Order Single User/Long Position/PNL < 0", async function () {
    ...
    const poolInfo11 = await poolFacet.getPool(xBtc);
    ...
    expect(initialMargin * BigInt(25005)).to.equals(
      positionInfo.initialMarginInUsd
    );
    expect(true).to.equals(positionInfo.isLong);
    expect(initialMargin * BigInt(10) * BigInt(25005)).to.equals(
      positionInfo.qty
    );
    ...
    const closeFeeInUsd = precision.mulRate(
      positionInfo.qty,
      symbolInfo.config.closeFeeRate
    );
    ...
    const initialMarginInUsd = precision.mulPrice(
      orderMargin1 - openFee,
      btcPrice1
    );
    ...
    const fundingRatePerSecond =
      (config.tradeConfig.fundingFeeBaseRate * positionInfo.qty) /
      positionInfo.qty;
    const fundingFeeInUsd =
      positionInfo.qty * fundingRatePerSecond * BigInt(time2 - time1);
    const longFundingFeePerQtyDelta = precision.divPrice(
      fundingFeeInUsd / positionInfo.qty,
      btcPrice2
    );

    const shortFundingFeePerQtyDelta = -fundingFeeInUsd / positionInfo.qty;
    const realizedFundingFeeDelta =
      (positionInfo.qty * longFundingFeePerQtyDelta) / BigInt(10 ** 18);

    const realizedFundingFee = precision.mulPrice(
    ...
    const borrowingPerTokenDelta =
      BigInt(time2 - time1) *
      ((poolConfig.baseInterestRate * poolInfo11.baseTokenBalance.holdAmount) /
        poolInfo11.baseTokenBalance.amount);

    const realizedBorrowingFee =
      (borrowingPerTokenDelta *
        precision.mulRate(initialMargin, precision.rate(10 - 1))) /
      BigInt(10 ** 18);
    const realizedBorrowingFeeInUsd = precision.mulPrice(
      realizedBorrowingFee,
      btcPrice2
    );
    const settleMargin = precision.divPrice(
      initialMarginInUsd -
        realizedFundingFee -
        realizedBorrowingFeeInUsd -
        closeFeeInUsd +
        lossInUsd,
      btcPrice2
    );
    ...
    // wallet
    expect(orderMargin1 - settleMargin).to.equals(
      nextMarketBalance - preMarketBalance
    );
    expect(0).to.equals(nextVaultBalance - preVaultBalance);
    expect(orderMargin1 - settleMargin).to.equals(
      preTokenBalance - nextTokenBalance
    );
  ...
  //   );
  //   expect(initialMargin).to.equals(positionInfo.initialMargin);
  //   expect(precision.mulPrice(initialMargin, ethPrice0)).to.equals(
  //     positionInfo.initialMarginInUsd
  //   );
  //   expect(true).to.equals(positionInfo.isLong);
  //   expect(precision.mulPrice(initialMargin * leverage, ethPrice0)).to.equals(
  //     positionInfo.qty
  //   );
  ...
  //   const closeFee1 = precision.divPrice(
  //     precision.mulRate(closeQty1, symbolInfo.config.closeFeeRate),
  //     ethPrice1
  //   );
  ...
  //   );
  //   expect(
  //     positionInfo.initialMargin -
  //       (positionInfo.initialMargin * closeQty1) / positionInfo.qty
  //   ).to.equals(positionInfo1.initialMargin);
  //   expect(
  //     positionInfo.initialMarginInUsd -
  //       (positionInfo.initialMarginInUsd * closeQty1) / positionInfo.qty
  //   ).to.equals(positionInfo1.initialMarginInUsd);
  //   expect(
  //     positionInfo.initialMarginInUsdFromBalance -
  //       (positionInfo.initialMarginInUsdFromBalance * closeQty1) /
  //         positionInfo.qty
  ...
  //     ((initialMarginInUsd - closeFee1InUsd + pnlInUsd) * closeQty1) /
  //       positionInfo.qty,
  //     ethPrice1
  //   );
  //   const userPnl1 =
  //     settledMargin1 - (initialMargin * closeQty1) / positionInfo.qty;
  ...
  //   // wallet
  //   expect(orderMargin1 - settledMargin1).to.equals(
  //     nextMarketBalance1 - preMarketBalance
  //   );
  //   expect(orderMargin1 - settledMargin1).to.equals(
  //     preTokenBalance - nextTokenBalance1
  //   );

  //   // pool
  //   const poolInfo1 = await poolFacet.getPool(xEth);
  //   expect(userPnl1 + closeFee1).to.equals(
  //     poolInfo.baseTokenBalance.amount - poolInfo1.baseTokenBalance.amount
  //   );

  //   expect(
  //     poolInfo.baseTokenBalance.holdAmount -
  //       (poolInfo.baseTokenBalance.holdAmount * closeQty1) / positionInfo.qty
  ...
  //   );
  //   expect(positionInfo.qty - closeQty1).to.equals(
  //     marketInfo.longPositionInterest
  //   );
  ...
  //   const closeFee2InUsd = precision.mulRate(
  //     closeQty2,
  //     symbolInfo.config.closeFeeRate
  //   );
  ...
  //       (positionInfo.initialMargin * closeQty2) / positionInfo.qty
  //   ).to.equals(positionInfo2.initialMargin);
  //   expect(
  //     positionInfo1.initialMarginInUsd -
  //       (positionInfo.initialMarginInUsd * closeQty2) / positionInfo.qty
  //   ).to.equals(positionInfo2.initialMarginInUsd);
  //   expect(
  //     positionInfo1.initialMarginInUsdFromBalance -
  //       (positionInfo.initialMarginInUsdFromBalance * closeQty2) /
  //         positionInfo.qty
  ...
  //     ((positionInfo1.initialMarginInUsd - closeFee2InUsd + pnlInUsd2) *
  //       closeQty2) /
  //       positionInfo1.qty,
  //     ethPrice2,
  //     18
  ...
  //   const userPnl2 =
  //     settledMargin2 -
  //     (positionInfo1.initialMargin * closeQty2) / positionInfo1.qty;
  ...
  //     poolInfo1.baseTokenBalance.amount - poolInfo2.baseTokenBalance.amount
  //   );

  //   expect(
  //     poolInfo1.baseTokenBalance.holdAmount -
  //       (poolInfo1.baseTokenBalance.holdAmount * closeQty2) / positionInfo1.qty
  //   ).to.equals(poolInfo2.baseTokenBalance.holdAmount);

  //   // Market
  //   const marketInfo2 = await marketFacet.getMarketInfo(
  //     ethUsd,
  //     oracles.format(ethOracle2)
  //   );
  //   expect(positionInfo1.qty - closeQty2).to.equals(
  //     marketInfo2.longPositionInterest
  //   );
  ...
  //   const closeFee3InUsd = precision.mulRate(
  //     closeQty3,
  //     symbolInfo.config.closeFeeRate
  //   );
  ...
  //   // position
  //   const positionInfo3 = await positionFacet.getSinglePosition(
  //     user0.address,
  //     ethUsd,
  //     wethAddr,
  //     defaultMarginMode
  ...
  //   const settledMargin3 = precision.divPrice(
  //     positionInfo2.initialMarginInUsd - closeFee3InUsd + pnlInUsd3,
  //     ethPrice3
  //   );
  ...
  //     poolInfo2.baseTokenBalance.amount - poolInfo3.baseTokenBalance.amount
  //   );
  ...
  //     (precision.divPrice(
  //       initialMarginInUsd - closeFee1InUsd + pnlInUsd,
  //       ethPrice1
  //     ) *
  //       closeQty1) /
  //     positionInfo.qty;
  ...
  //   const closeFee3 = precision.divPrice(
  //     precision.mulRate(closeQty3, symbolInfo.config.closeFeeRate),
  //     ethPrice3
  //   );
  ...
  //   expect(
  //     precision.usd(precision.mulPrice(initialMargin, usdtPrice0), 18 - 6)
  //   ).to.equals(positionInfo.initialMarginInUsd);
  //   expect(false).to.equals(positionInfo.isLong);
  //   expect(
  //     precision.usd(
  //       precision.mulPrice(initialMargin * leverage, usdtPrice0),
  //       18 - 6
  //     )
  //   ).to.equals(positionInfo.qty);

  //   const nextMarketBalance0 = BigInt(await usdc.balanceOf(xEth));
  //   const nextPfVaultBalance0 = BigInt(
  //     await usdc.balanceOf(portfolioVaultAddr)
  //   );
  ...
  //       usdtPrice1
  //     ) / BigInt(10 ** (18 - 6));
  //   const closeFee1InUsd = precision.usd(
  //     precision.mulPrice(closeFee1, usdtPrice1),
  //     18 - 6
  //   );
  ...
  //   const accountInfo1 = await accountFacet.getAccountInfoWithOracles(
  //     user0.address,
  //     oracles.format(oracle1)
  //   );

  //   const pnlInUsd = (positionInfo.qty * (ethPrice0 - ethPrice1)) / ethPrice0;
  //   const initialMarginInUsd = precision.usd(
  //     precision.mulPrice(initialMargin, usdtPrice0),
  //     18 - 6
  //   );
  ...
  //   const nextPfVaultBalance1 = BigInt(
  //     await usdc.balanceOf(portfolioVaultAddr)
  //   );
  ...
  //   // wallet
  //   expect(closeFee1 + userPnl1 - settledMargin1).to.equals(
  //     nextMarketBalance1 - nextMarketBalance0
  //   );
  //   expect(closeFee1 + userPnl1).to.equals(
  //     preUsdVaultBalance - nextUsdVaultBalance1
  //   );
  ...
  //   expect(userPnl1 + closeFee1).to.equals(
  //     xtokenPoolInfo1.stableTokenBalances[0].lossAmount
  //   );
  //   expect(userPnl1 + closeFee1).to.equals(
  //     xUsdPoolInfo.stableTokenBalances[0].amount -
  //       xUsdPoolInfo1.stableTokenBalances[0].amount
  //   );
  //   expect(userPnl1 + closeFee1).to.equals(
  //     xUsdPoolInfo1.stableTokenBalances[0].unsettledAmount -
  //       xUsdPoolInfo.stableTokenBalances[0].unsettledAmount
  //   );
  //   expect(
  //     xUsdPoolInfo.stableTokenBalances[0].holdAmount -
  //       (xUsdPoolInfo.stableTokenBalances[0].holdAmount * closeQty1) /
  //         positionInfo.qty
  ...
  //   );
  //   expect(positionInfo.qty - closeQty1).to.equals(
  //     marketInfo.totalShortPositionInterest
  //   );
  ...
  //   const accountInfo2 = await accountFacet.getAccountInfoWithOracles(
  //     user0.address,
  //     oracles.format(oracle2)
  //   );
  //   const nextMarketBalance2 = BigInt(await usdc.balanceOf(xEth));
  //   const nextPfVaultBalance2 = BigInt(
  //     await usdc.balanceOf(portfolioVaultAddr)
  //   );
  ...
  //   const closeFee2 =
  //     precision.divPrice(
  //       precision.mulRate(closeQty2, symbolInfo.config.closeFeeRate),
  //       usdtPrice2
  //     ) / BigInt(10 ** 12);
  //   const closeFee2InUsd = precision.usd(
  //     precision.mulPrice(closeFee2, usdtPrice2),
  //     18 - 6
  //   );
  ...
  //   expect(
  //     positionInfo1.initialMarginInUsd -
  //       (positionInfo1.initialMarginInUsd * closeQty2) / positionInfo1.qty
  //   ).to.equals(positionInfo2.initialMarginInUsd);
  //   expect(
  //     positionInfo1.initialMarginInUsdFromBalance -
  //       (positionInfo1.initialMarginInUsdFromBalance * closeQty2) /
  //         positionInfo1.qty
  //   ).to.equals(positionInfo2.initialMarginInUsdFromBalance);
  //   expect(ethPrice0).to.equals(positionInfo2.entryPrice);
  //   expect(positionInfo1.qty - closeQty2).to.equals(positionInfo2.qty);
  ...
  //   // wallet
  //   expect(closeFee2 + userPnl2 - settledMargin2).to.equals(
  //     nextMarketBalance2 - nextMarketBalance1
  //   );
  //   expect(closeFee2 + userPnl2).to.equals(
  //     nextUsdVaultBalance1 - nextUsdVaultBalance2
  //   );
  //   expect(settledMargin2).to.equals(nextTokenBalance2 - nextTokenBalance1);
  ...
  //   expect(userPnl2 + closeFee2).to.equals(
  //     xtokenPoolInfo2.stableTokenBalances[0].lossAmount -
  //       xtokenPoolInfo1.stableTokenBalances[0].lossAmount
  //   );
  //   expect(userPnl2 + closeFee2).to.equals(
  //     xUsdPoolInfo1.stableTokenBalances[0].amount -
  //       xUsdPoolInfo2.stableTokenBalances[0].amount
  //   );
  //   expect(userPnl2 + closeFee2).to.equals(
  //     xUsdPoolInfo2.stableTokenBalances[0].unsettledAmount -
  //       xUsdPoolInfo1.stableTokenBalances[0].unsettledAmount
  //   );
  //   expect(
  //     xUsdPoolInfo1.stableTokenBalances[0].holdAmount -
  //       (xUsdPoolInfo1.stableTokenBalances[0].holdAmount * closeQty2) /
  //         positionInfo1.qty
  ...
  //   );
  //   expect(positionInfo1.qty - closeQty2).to.equals(
  //     marketInfo2.totalShortPositionInterest
  //   );
  ...
  //   const accountInfo3 = await accountFacet.getAccountInfoWithOracles(
  //     user0.address,
  ...
  //       usdtPrice3
  //     ) / BigInt(10 ** 12);
  //   const closeFee3InUsd = precision.usd(
  //     precision.mulPrice(closeFee3, usdtPrice3),
  //     18 - 6
  //   );
  ...
  //   const nextPfVaultBalance3 = BigInt(
  //     await usdc.balanceOf(portfolioVaultAddr)
  //   );
  ...
  //   expect(closeFee3 + userPnl3).to.equals(
  //     nextUsdVaultBalance2 - nextUsdVaultBalance3
  //   );
  //   expect(closeFee3 + userPnl3 - settledMargin3).to.equals(
  ...
  //     xtokenPoolInfo3.stableTokenBalances[0].lossAmount -
  //       xtokenPoolInfo2.stableTokenBalances[0].lossAmount
  //   );
  //   expect(userPnl3 + closeFee3).to.equals(
  //     xUsdPoolInfo2.stableTokenBalances[0].amount -
  //       xUsdPoolInfo3.stableTokenBalances[0].amount
  //   );
  //   expect(userPnl3 + closeFee3).to.equals(
  //     xUsdPoolInfo3.stableTokenBalances[0].unsettledAmount -
  //       xUsdPoolInfo2.stableTokenBalances[0].unsettledAmount
  //   );
  ...
  //   const closeFee1InUsd = precision.mulRate(
  //     closeQty1,
  //     symbolInfo.config.closeFeeRate
  //   );
  //   const closeFee1 =
  //     precision.divPrice(closeFee1InUsd, usdtPrice1) / BigInt(10 ** (18 - 6));
  ...
  //   );
  //   const closeFee2 =
  //     precision.divPrice(closeFee2InUsd, usdtPrice2) / BigInt(10 ** 12);
  ...
  //   expect(-userPnl2).to.equals(
  //     xtokenPoolInfo2.stableTokenBalances[0].amount -
  //       xtokenPoolInfo1.stableTokenBalances[0].amount
  //   );
  //   expect(0).to.equals(
  //     xUsdPoolInfo1.stableTokenBalances[0].amount -
  //       xUsdPoolInfo2.stableTokenBalances[0].amount
  ...
  //   );
  //   const closeFee3 =
  //     precision.divPrice(closeFee3InUsd, usdtPrice3) / BigInt(10 ** 12);
  ...
  //   expect(-userPnl3).to.equals(
  //     xtokenPoolInfo3.stableTokenBalances[0].amount -
  //       xtokenPoolInfo2.stableTokenBalances[0].amount
  //   );
  //   expect(0).to.equals(
  //     xUsdPoolInfo2.stableTokenBalances[0].amount -
